...
class PoolManager {
    ...
    async initialize(ctx) {
        if (this.initialized)
            return;
        this.logger = ctx.log;
        const coordinationDir = ctx.config.get('integrations.agentPool.coordinationDir')
            || join(process.cwd(), '../thetadrivencoach/openclaw/data/coordination');
        const poolSize = ctx.config.get('integrations.agentPool.poolSize') || 50;
        const maxRetries = ctx.config.get('integrations.agentPool.maxRetries') || 3;
        const taskTimeoutMs = ctx.config.get('integrations.agentPool.taskTimeoutMs') || 10 * 60 * 1000;
 ...
 */
export async function executeParallel(request, ctx, currentSovereignty) {
    ...
    if (currentSovereignty < requiredSovereignty) {
        throw new Error(`Insufficient sovereignty for parallel ${request.operation}. ` +
            `Required: ${requiredSovereignty}, Current: ${currentSovereignty}`);
    }
    ctx.log.info(`Starting parallel ${request.operation}: ${request.description} ` +
    ...
    for (let i = 0; i < subtaskSpecs.length; i += maxConcurrency) {
        ...
        const batchResults = await Promise.allSettled(batchTaskIds.map(taskId => pool.waitForTask(taskId)));
    ...
    ctx.log.info(`Parallel ${request.operation} completed: ${completedCount}/${subtaskResults.length} succeeded ` +
 ...
 */
export function canExecuteParallel(operation, currentSovereignty, poolAvailable) {
    if (!poolAvailable) {
 ...
 */
export async function suggestParallelTasks(specPath, repoRoot) {
...
// Private Helpers
// ═══════════════════════════════════════════════════════════════
function getSovereigntyRequirement(operation) {
    switch (operation) {
        case 'analyze':
...
function generatePrompt(operation, description, files) {
 ...
 */
export async function registerParallelTasksWithScheduler(specPath, repoRoot, onTaskReady) {
...
// ═══════════════════════════════════════════════════════════════
export { PoolManager };
//# sourceMappingURL=pool-integration.js.map