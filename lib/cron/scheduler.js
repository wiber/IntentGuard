...
// ═══════════════════════════════════════════════════════════════
function buildTaskRegistry(repoRoot) {
    return [
        // ─── SAFE: Test Coverage ────────────────────────────
        {
...
export class ProactiveScheduler {
    ...
    injectCallback = null;
    getSovereignty = () => 0.7;
    checkIdle = () => ({ idleMs: 0, runningTasks: 0 });
    scanSpec = () => 0;
    constructor(log, repoRoot, config = DEFAULT_SCHEDULER_CONFIG) {
        this.log = log;
        this.config = config;
     ...
     */
    bind(inject, getSovereignty, checkIdle, scanSpec) {
    ...
    async heartbeat() {
        ...
        const eligible = this.tasks.filter(task => {
            ...
            if (Date.now() - lastRun < task.cooldownMinutes * 60 * 1000)
                return false;
            // Check sovereignty threshold
            if (sovereignty < task.minSovereignty)
                return false;
        ...
        this.log.info(`[NightShift] Injecting: "${task.description}" | risk=${task.risk} tier=${tier} ` +
