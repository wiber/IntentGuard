...
import { intersection, autoIntersection, pivotalQuestion, detectCell, } from './discord/shortrank-notation.js';
...
function markSpecDone(todoText) {
    const specPath = join(ROOT, 'spec/sections/08-implementation-plan.tsx');
...
function addSpecTodos(phase, items) {
    const specPath = join(ROOT, 'spec/sections/08-implementation-plan.tsx');
    let content = readFileSync(specPath, 'utf-8');
...
function scoreTodo(todo) {
    ...
    if (t.includes('skeleton') || t.includes('create') || t.includes('build'))
        score += 20;
    if (t.includes('test') || t.includes('verify'))
        score += 10;
    if (t.includes('wire') || t.includes('connect'))
        score += 15;
    if (t.includes('add') || t.includes('implement'))
        score += 12;
    if (t.includes('command') || t.includes('!'))
        score += 8;
    // Simpler tasks get slight boost (likely faster to complete)
    if (todo.text.length < 60)
        score += 5;
    return score;
}
// ═══════════════════════════════════════════════════════════════
...
async function createSkeleton(todo, start) {
    const t = todo.text;
    // Extract file path from task text
    const pathMatch = t.match(/(?:src\/[\w\/-]+\.ts)/);
    if (!pathMatch)
        return { success: false, output: 'Could not extract file path', code: 1, durationMs: Date.now() - start };
...
async function addDiscordCommand(todo, start) {
    const cmdMatch = todo.text.match(/!(\w[\w-]*)/);
    if (!cmdMatch)
        return { success: false, output: 'Could not extract command name', code: 1, durationMs: Date.now() - start };
    ...
    content = content.replace(/(\s*case '!pipeline':)/, `${handler}\n$1`);
...
async function wireShortRankCell(todo, start) {
...
async function addChannel(todo, start) {
    const channelMatch = todo.text.match(/#([\w-]+)/);
    if (!channelMatch)
        return { success: false, output: 'Could not extract channel name', code: 1, durationMs: Date.now() - start };
    ...
    content = content.replace(/(\{ name: 'x-posts'[^}]+\},)/, `$1\n  { name: '${name}', description: '${desc}' },`);
...
async function createBuildTask(todo, start) {
    const t = todo.text.toLowerCase();
    // File creation tasks
...
function runShellTask(todo, start) {
    return new Promise((resolve) => {
        ...
        let output = '';
        child.stdout?.on('data', (d) => { if (output.length < 50000)
            output += d.toString(); });
        child.stderr?.on('data', (d) => { if (output.length < 50000)
            output += d.toString(); });
...
class DiscordTweeter {
    ...
    async connect() {
        ...
        if (!token || !guildId)
            throw new Error('Missing DISCORD_BOT_TOKEN or THETADRIVEN_GUILD_ID');
        return new Promise((resolve, reject) => {
            this.client.once('ready', () => {
                const guild = this.client.guilds.cache.get(guildId);
                if (!guild) {
                    reject(new Error(`Guild not found: ${guildId}`));
                    return;
                }
                for (const [, ch] of guild.channels.cache) {
                    if (ch.type !== ChannelType.GuildText)
                        continue;
                    const tc = ch;
                    if (tc.name === 'trust-debt-public')
                        this.tweetChannel = tc;
                    if (tc.name === 'x-posts')
                        this.xPostsChannel = tc;
                    for (const room of ['builder', 'architect', 'operator', 'vault', 'voice', 'laboratory', 'performer', 'navigator', 'network']) {
                        if (tc.name === room)
                            this.roomChannels.set(room, tc);
                    }
                }
                this.ready = true;
                resolve();
    ...
    async tweet(text) {
        if (!this.ready || !this.tweetChannel)
            return null;
        try {
    ...
    async xPost(text) {
        if (!this.ready || !this.xPostsChannel)
            return null;
        try {
            const msg = await this.xPostsChannel.send(`📝 **Draft Tweet** — React 👍 to publish to X\n\n${text}`);
            return msg.id;
        }
        catch {
            return null;
...
// ═══════════════════════════════════════════════════════════════
async function ceoLoop(config = DEFAULT_CONFIG) {
    console.log('═══════════════════════════════════════════════════');
    ...
    while (true) {
        ...
        const actionable = todos.filter(t => (!config.skipFuturePhases || !t.future) && !failedThisSession.has(t.text));
        // Sort by priority
...
for (const arg of args) {
    if (arg.startsWith('--cooldown='))
        config.cooldownSec = parseInt(arg.split('=')[1]);
    if (arg.startsWith('--idle-scan='))
        config.idleScanIntervalSec = parseInt(arg.split('=')[1]);
    if (arg.startsWith('--heartbeat='))
        config.heartbeatIntervalSec = parseInt(arg.split('=')[1]);
    if (arg === '--no-commit')
        config.autoCommit = false;
    if (arg === '--include-future')
        config.skipFuturePhases = false;
    if (arg === '--no-nightly')
        config.nightlySummary = false;
}
ceoLoop(config).catch((err) => {
    console.error('[CEO] Fatal error:', err);
    process.exit(1);
});
//# sourceMappingURL=ceo-loop.js.map