 ...
 */
export function calculateDailyLimit(sovereignty) {
 ...
 */
export function calculateSpendingLimit(sovereigntyCalc) {
 ...
 */
export function getSovereigntyLevel(sovereignty) {
    if (sovereignty >= 1.0)
        return 'PERFECT';
    if (sovereignty >= 0.9)
        return 'EXCELLENT';
 ...
 */
function getLevelName(threshold) {
 ...
 */
export function sovereigntyNeededForLimit(currentSovereignty, targetLimit) {
 ...
 */
export function calculateSpendingRecoveryPath(currentSovereignty) {
    const currentLimit = calculateDailyLimit(currentSovereignty);
    const path = [];
    const levels = [
        'RESTRICTED',
 ...
 */
export function formatSpendingLimitReport(calc) {
    const lines = [];
 ...
 */
export function formatRecoveryRecommendations(currentSovereignty) {
    const path = calculateSpendingRecoveryPath(currentSovereignty);
    if (path.length === 0) {
        return '✅ No recovery needed — operating at maximum sovereignty';
    }
    const lines = ['📈 Spending Limit Recovery Path:', ''];
    for (const milestone of path) {
        lines.push(`${SOVEREIGNTY_MILESTONES[milestone.level].emoji} ${milestone.level}: ` +
