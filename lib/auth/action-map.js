 ...
 */
export function getActionRequirement(toolName) {
    return ACTION_MAP[toolName];
}
/**
 * Check if a tool is registered in the action map.
 */
export function hasActionRequirement(toolName) {
 ...
 */
export function getActionsBySovereignty(minSovereignty) {
 ...
 */
export function getRiskLevel(minSovereignty) {
    if (minSovereignty >= 0.9)
        return 'critical';
    if (minSovereignty >= 0.7)
        return 'high';
    if (minSovereignty >= 0.4)
        return 'medium';
    return 'low';
}
