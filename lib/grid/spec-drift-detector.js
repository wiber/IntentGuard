...
// Spec Intent Scanner
// ═══════════════════════════════════════════════════════════════
function scanSpecIntent(specContent, comsContent, mapping) {
    ...
    let specStatus = 'not_mentioned';
    if (mentions > 10) {
        // Check if there are active/pending items
        const hasActive = priorities.some(p => p.includes('- [ ]') || p.includes('pending') || p.includes('critical') || p.includes('❌'));
        const hasCompleted = priorities.some(p => p.includes('- [x]') || p.includes('✅') || p.includes('complete'));
        if (hasActive)
            specStatus = 'active';
...
// ═══════════════════════════════════════════════════════════════
function scanRepoReality(repoRoot, mapping) {
    let recentCommits = 0;
    let fileCount = 0;
    let lastModified = '';
    let linesOfCode = 0;
    ...
    for (const repoPath of mapping.repoPaths) {
        if (repoPath.includes('*'))
            continue;
        try {
            const result = execSync(`git log --oneline --since="30 days ago" -- "${repoPath}" 2>/dev/null | wc -l`, { cwd: repoRoot, encoding: 'utf-8', timeout: 5000 }).trim();
...
// Drift Calculation
// ═══════════════════════════════════════════════════════════════
function calculateDrift(intent, reality) {
...
// ═══════════════════════════════════════════════════════════════
function renderDriftGrid(cells) {
...
// ═══════════════════════════════════════════════════════════════
export function generateDriftSignal(repoRoot, specPath, comsPath) {
...
//# sourceMappingURL=spec-drift-detector.js.map