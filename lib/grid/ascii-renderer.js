...
// Heat Coloring Logic
// ═══════════════════════════════════════════════════════════════
function getHeatColor(pressure) {
    if (pressure < 0.3)
        return ANSI.green;
    if (pressure < 0.7)
        return ANSI.yellow;
...
function getHeatEmoji(pressure) {
    if (pressure < 0.3)
        return '🟢';
    if (pressure < 0.7)
        return '🟡';
...
export function renderGrid(cellPressures) {
    const lines = [];
    // Header
...
function renderRow(cells, pressures) {
...
export function renderGridEmbed(cellPressures, recentEvents) {
...
function buildGridDescription(cellPressures) {
    const lines = [];
    for (let row = 0; row < 3; row++) {
        const rowLabel = ROW_LABELS[row];
        const cells = GRID_CELLS.filter(c => c.row === row);
