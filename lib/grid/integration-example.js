 ...
 */
import { gridEventBridge, hotCellRouter, fetchGridState, pushPointerEvent, renderGrid, renderGridEmbed, deepLinker, } from './index.js';
// ═══════════════════════════════════════════════════════════════
// Example 1: Complete Task Completion Flow
// ═══════════════════════════════════════════════════════════════
export async function handleTaskCompletion(phase, taskText, discordChannelId, discordMessageId) {
...
export async function handleGridStatusCommand() {
    ...
    const recentEvents = hotCellRouter
        .getAllPressures()
        .slice(0, 5)
        .map(cell => ({
        cellId: cell.cell,
...
// ═══════════════════════════════════════════════════════════════
export async function checkForHotCells() {
    console.log(`\n🔥 Hot Cell Check`);
    // 1. Update pressures
    const pressures = hotCellRouter.updatePressures();
...
// ═══════════════════════════════════════════════════════════════
export async function processBatchTasks(tasks) {
...
// ═══════════════════════════════════════════════════════════════
export async function generateTimelineView(cellId, hoursBack = 24) {
    console.log(`\n📅 Timeline View: ${cellId} (last ${hoursBack}h)`);
    const startTime = new Date(Date.now() - hoursBack * 60 * 60 * 1000);
    const endTime = new Date();
    // 1. Generate timeline deep link
    const timelineLink = deepLinker.generateTimelineLink(cellId, startTime, endTime);
...
export async function compareHotCells() {
    ...
    const cellLines = cellData.map(c => `• **${c.cell}**: ${(c.pressure * 100).toFixed(0)}% (${c.events} events)`);
    const message = `
🔍 **Hot Cell Comparison**
...
export async function fullIntegrationDemo() {
    ...
    await handleTaskCompletion(0, 'Define Q1 revenue goals', 'channel-123', 'message-456');
    // 2. Get grid status
