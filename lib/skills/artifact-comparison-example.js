...
import { saveArtifactSnapshot, loadArtifactSnapshot, compareArtifacts, generateAsciiDiff, compareAgainstHistory, listArtifactSnapshots, } from './artifact-comparison.js';
 ...
 */
export function generateAndSaveArtifact(identityVector, sovereignty) {
 ...
 */
export function checkForSignificantChange(identityVector, sovereignty) {
    ...
    const hasChanged = Math.abs(diff.metrics.sovereigntyDelta) > significantSovereigntyDelta ||
 ...
 */
export function generateDiscordReport(timestamp1, timestamp2) {
 ...
 */
export function generateHistorySummary(limit = 5) {
 ...
 */
export async function maybeGenerateArtifact(identityVector, sovereignty, discordNotify) {
 ...
 */
export function handleCompareCommand(args) {
    if (args.length < 2) {
        const snapshots = listArtifactSnapshots();
        if (snapshots.length < 2) {
 ...
 */
export function checkStabilityMilestone(daysThreshold = 30) {
    const snapshots = listArtifactSnapshots();
    if (snapshots.length < 2) {
...
//# sourceMappingURL=artifact-comparison-example.js.map