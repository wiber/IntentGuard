...
export default class ArtifactGenerator {
    ...
    async execute(command, ctx) {
        ...
        switch (cmd.action) {
            case 'generate':
                return this.generateArtifact(cmd.identity);
            case 'compare':
                return this.compareArtifacts(cmd.pathA, cmd.pathB);
            default:
     ...
     *   5. Save metadata
     */
    async generateArtifact(identity) {
        try {
     ...
     */
    async compareArtifacts(pathA, pathB) {
        try {
    ...
    identityToVector(identity) {
        ...
        return categories.map(cat => identity.categoryScores[cat] ?? 0.5);
    }
    // ─── Helper: Comparison Description ──────────────────────────────────
    generateComparisonDescription(metaA, metaB, diff) {
        const lines = [];
