...
export default class LLMControllerSkill {
    name = 'llm-controller';
    ...
    usageLogPath = '';
    // Routing thresholds
    ollamaMaxComplexity = 2; // Ollama handles complexity 1-2
    sonnetMaxComplexity = 4; // Sonnet handles complexity 3-4
    confidenceThreshold = 0.7; // Below this, escalate to next tier
    async initialize(ctx) {
        this.ollamaEndpoint = ctx.config.get('integrations.ollama.endpoint')
     ...
     */
    trackCost(model, inputTokens, outputTokens, backend) {
        if (this.costReporter) {
    ...
    routeRequest(req, ctx) {
        ...
        if (req.backend && req.backend !== 'auto') {
            ...
            return { backend: req.backend, reason: 'Explicit backend requested', tier: tier };
        }
    ...
    async execute(command, ctx) {
        const req = command;
        if (req.audioUrl)
            return this.transcribe(req, ctx);
        // Cost Governor check
     ...
     */
    async escalateToHuman(req, reason, ctx) {
        const humanRequest = {
    ...
    async runBoth(req, ctx) {
        ...
        if (ollama.status === 'fulfilled')
            results.push(ollama.value);
        if (sonnet.status === 'fulfilled')
            results.push(sonnet.value);
        return {
    ...
    async transcribeViaWhisper(audioPath, ctx) {
        const start = Date.now();
        const outputDir = join(this.rootDir, 'data', 'audio-tmp');
        const result = await ctx.shell.exec(`PATH="/Users/thetadriven/Library/Python/3.9/bin:$PATH" whisper "${audioPath}" --model tiny --output_format json --output_dir "${outputDir}" --language en 2>/dev/null`);
        if (result.code !== 0)
            throw new Error(`Whisper exit ${result.code}`);
    ...
    async transcribeViaAPI(audioBuffer, prompt, ctx) {
        const start = Date.now();
    ...
    async transcribeViaCLI(audioPath, prompt, ctx) {
        const start = Date.now();
        const escaped = prompt.replace(/'/g, "'\\''");
        const result = await ctx.shell.exec(`unset CLAUDECODE && claude -p 'Read the audio file at ${audioPath} and ${escaped}' --max-turns 1 2>&1`);
        return {
    ...
    async callOllama(prompt, systemPrompt, ctx) {
        const start = Date.now();
    ...
    async callSonnet(prompt, systemPrompt, ctx) {
        if (this.anthropicKey) {
            const start = Date.now();
            const body = {
                model: 'claude-sonnet-4-20250514',
