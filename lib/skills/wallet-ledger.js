...
export default class WalletLedger {
     ...
     */
    trackInferenceCost(model, tokens, sovereignty, isOllama = false) {
        const cost = isOllama
            ? this.estimateOllamaCost(tokens)
            : this.estimateApiCost(model, tokens);
        if (cost > 0) {
            this.appendTransaction('expense', cost, 'inference', `${model} inference (${tokens} tokens)`, sovereignty);
        }
    }
    /**
...
//# sourceMappingURL=wallet-ledger.js.map