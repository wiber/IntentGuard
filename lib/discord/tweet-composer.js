 ...
 */
import { autoIntersection, intersection, detectCell, trustCategoriesToCell, pivotalQuestion, } from './shortrank-notation.js';
export class TweetComposer {
     ...
     */
    bind(discord, primaryChannelId, gameChannelId, xPostsChannelId) {
     ...
     */
    async post(data) {
        if (!this.discord || !this.primaryChannelId)
            return null;
        ...
        try {
            ...
            if (this.onPivotalQuestion) {
                ...
                await this.onPivotalQuestion(pq.room, `**Pivotal Question:** ${pq.question}`, `**Predicted Answer:** ${pq.predictedAnswer}`);
            }
            return message;
        }
        catch (err) {
     ...
     */
    async crossPostToGame(tweetId) {
        if (!this.discord || !this.gameChannelId)
            return false;
     ...
     */
    async handleReaction(messageId, emoji, isAdmin) {
        if (!isAdmin)
            return null;
     ...
     */
    pipelineTweet(sovereigntyScore, alignmentPercent, trustDebtUnits, categoriesActive) {
        return {
     ...
     */
    taskTweet(room, taskSummary, success, sovereignty) {
        return {
            text: success
     ...
     */
    fimDenialTweet(toolName, overlap, sovereignty) {
        return {
     ...
     */
    predictionTweet(room, action, timeoutSec, sovereignty) {
        return {
     ...
     */
    driftTweet(category, driftPercent, sovereignty) {
        return {
     ...
     */
    intelligenceBurst(room, action, result, sovereignty, hardnessTier, fimOverlap, patentRef) {
     ...
     */
    costGovernorTweet(dailySpend, budget, switchedTo, sovereignty) {
        return {
     ...
     */
    heartbeatTweet(sovereignty, activeTasks, dailySpend, uptimeHours) {
        return {
...
//# sourceMappingURL=tweet-composer.js.map