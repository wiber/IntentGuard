...
export class TaskStore {
    ...
    setDiscordMessageId(id, messageId) {
        const task = this.tasks.get(id);
        if (task)
    ...
    getRunningTaskForRoom(room) {
        return [...this.tasks.values()].find(t => t.room === room && (t.status === 'dispatched' || t.status === 'running'));
    }
    getRecent(count = 10) {
            ...
            for (const line of lines) {
                ...
                if (entry.type === 'create' && entry.task) {
                    ...
                    if (task) {
                        ...
                        if (entry.extra)
                            Object.assign(task, entry.extra);
                    }
                }
            }
