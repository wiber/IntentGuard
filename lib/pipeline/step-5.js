 ...
 */
function extractGitHistory(repoPath) {
    try {
        // Get git log with format: ISO8601 date|commit hash|commit message
        const gitLog = execSync('git log --all --date=iso-strict --pretty=format:"%ad|%H|%s" --no-merges', { cwd: repoPath, encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024 });
        if (!gitLog.trim())
            return [];
        return gitLog
            .trim()
 ...
 */
function buildTimelineSnapshots(gitHistory, keywords) {
    const snapshots = [];
 ...
 */
function analyzeCategoryTrends(snapshots) {
    ...
    for (const [category, data] of Object.entries(categoryData)) {
        ...
        const trend = Math.abs(changeRate) < 10 ? 'stable' :
 ...
 */
function generateInsights(trends, snapshots, dateRange) {
    const insights = [];
...
export async function run(runDir, stepDir) {
    ...
    const recommendations = trends
        ...
        .slice(0, 5)
        .map(t => {
        const priority = t.recentActivity === 0 ? 'high' : 'medium';
        return {
            priority,
            category: t.category,
    ...
    writeFileSync(join(stepDir, '5-timeline-history.json'), JSON.stringify(result, null, 2));
...
//# sourceMappingURL=step-5.js.map