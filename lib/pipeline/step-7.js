 ...
 */
function validateCategoryOrdering(categories) {
    try {
            ...
            if (Array.isArray(row)) {
                for (const cell of row) {
                    if (typeof cell === 'number') {
                        ...
                        totalValue += Math.abs(Number(cell.value) || 0);
                    }
                }
            }
        }
 ...
 */
function validatePipelineIntegrity(stepsLoaded) {
 ...
 */
function validateScoreRanges(sovereigntyScore, alignmentScore) {
    const issues = [];
 ...
 */
function loadStep(runDir, stepNum, fileName) {
    const stepName = [
...
export async function run(runDir, stepDir) {
    ...
    const step0 = loadStep(runDir, 0, '0-raw-materials.json');
    const step1 = loadStep(runDir, 1, '1-document-processing.json');
    const step2 = loadStep(runDir, 2, '2-organic-extraction.json');
    const step3 = loadStep(runDir, 3, '3-frequency-analysis.json');
    const step4 = loadStep(runDir, 4, '4-grades-statistics.json');
    const step5 = loadStep(runDir, 5, '5-goal-alignment.json');
    const step6 = loadStep(runDir, 6, '6-symmetric-matrix.json');
    ...
    const gradeCategories = step4?.categories;
    const drifts = (step5?.drifts || []);
    ...
    // Write audit log
    writeFileSync(join(stepDir, '7-audit-log.json'), JSON.stringify(auditLog, null, 2));
    ...
    // Write JSON report
    writeFileSync(join(stepDir, '7-final-report.json'), JSON.stringify(report, null, 2));
    // Write HTML report
...
//# sourceMappingURL=step-7.js.map