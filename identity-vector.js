...
exports.loadIdentityVector = loadIdentityVector;
exports.loadIdentityVectorDefault = loadIdentityVectorDefault;
exports.normalizeRawVector = normalizeRawVector;
exports.vectorToRaw = vectorToRaw;
exports.clearCache = clearCache;
exports.getCacheStats = getCacheStats;
const fs_1 = require("fs");
const path_1 = require("path");
const geometric_js_1 = require("./geometric.js");
// ─── Constants ──────────────────────────────────────────────────────────
/**
 ...
 */
const PIPELINE_CATEGORY_MAPPING = {
 ...
 */
function setCache(cacheKey, vector, sourceFile) {
    cache.set(cacheKey, {
        vector,
        loadedAt: Date.now(),
        sourceFile,
 ...
 * ```
 */
function loadIdentityVector(step4Path, userId = 'system') {
    // Check cache first
    ...
    if (!(0, fs_1.existsSync)(step4Path)) {
        throw new Error(`Step-4 output not found: ${step4Path}`);
    }
    // Load and parse JSON
    const rawData = (0, fs_1.readFileSync)(step4Path, 'utf-8');
    ...
    for (const cat of geometric_js_1.TRUST_DEBT_CATEGORIES) {
    ...
    const orthogonalityScore = data.statistical_analysis.orthogonality_score / 100;
 ...
 * ```
 */
function loadIdentityVectorDefault(userId = 'system') {
    const possiblePaths = [
        '4-grades-statistics.json',
        (0, path_1.join)(process.cwd(), '4-grades-statistics.json'),
        (0, path_1.join)(process.cwd(), 'data', 'pipeline-output', '4-grades-statistics.json'),
    ];
 ...
 * ```
 */
function normalizeRawVector(rawVector, userId = 'system') {
    if (rawVector.length !== 20) {
        throw new Error(`Expected 20-dimensional vector, got ${rawVector.length}`);
    }
    const categoryScores = {};
    geometric_js_1.TRUST_DEBT_CATEGORIES.forEach((cat, i) => {
 ...
 * ```
 */
function vectorToRaw(vector) {
    return geometric_js_1.TRUST_DEBT_CATEGORIES.map(cat => vector.categoryScores[cat] ?? 0.5);
}
/**
