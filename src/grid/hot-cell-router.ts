...
export class HotCellRouter {
  ...
  private cellPressures: Map<string, CellPressure> = new Map();
  ...
  private loadEvents(): GridEvent[] {
    ...
    try {
      ...
      return lines.map(line => JSON.parse(line) as GridEvent);
    } catch (error) {
  ...
  updatePressures(events?: GridEvent[]): Record<string, number> {
    ...
    const cellData: Map<string, { score: number; count: number; lastEvent: string }> = new Map();
    ...
    for (const event of eventList) {
      ...
      if (age > ONE_DAY) continue; // Ignore events older than 24 hours

      let weight = 0.2; // Default weight for last 24 hours
      if (age < ONE_HOUR) weight = 1.0;
      else if (age < SIX_HOURS) weight = 0.5;
   ...
   */
  getHotCells(threshold: number = 0.7): string[] {
  ...
  routeToRoom(hotCells?: string[]): RoutingRecommendation {
    ...
    const roomPressures: Map<string, { cells: string[]; totalPressure: number }> = new Map();
  ...
  getSummary(): string {
    ...
    for (const cell of pressures) {
      ...
      lines.push(
        `  ${cell.cell} (${cell.room}): ${bar} ${(cell.pressure * 100).toFixed(0)}% (${cell.eventCount} events)`
      );
    }
