...
import { fetchGridState, pushPointerEvent, type PointerEventResponse } from '../grid/tesseract-client.js';
...
function checkRateLimit(ip: string, config: PlaygroundConfig): boolean {
...
function setCorsHeaders(res: ServerResponse, config: PlaygroundConfig): void {
...
async function handleGetGridState(
  req: IncomingMessage,
  res: ServerResponse,
  config: PlaygroundConfig
): Promise<void> {
...
async function handlePostPointerEvent(
  req: IncomingMessage,
  res: ServerResponse,
  config: PlaygroundConfig
): Promise<void> {
  try {
    ...
    const result: PointerEventResponse = await pushPointerEvent(
      body.cellId,
      body.eventType,
      body.data || {}
    );
...
async function handlePostBatchPointerEvents(
  req: IncomingMessage,
  res: ServerResponse,
  config: PlaygroundConfig
): Promise<void> {
  try {
    ...
    const results: PointerEventResponse[] = [];
...
function handlePlaygroundInterface(
  req: IncomingMessage,
  res: ServerResponse,
  config: PlaygroundConfig
): void {
...
export async function startPlaygroundServer(config?: Partial<PlaygroundConfig>): Promise<TesseractPlayground> {
