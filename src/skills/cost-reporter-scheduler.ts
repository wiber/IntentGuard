...
export interface CostReporterSchedulerConfig {
  enabled: boolean;
  dailyReportHour: number; // UTC hour (0-23)
  weeklyReportDay: number; // Day of week (0=Sunday, 1=Monday, etc.)
  weeklyReportHour: number; // UTC hour (0-23)
  minIntervalBetweenReportsMs: number; // Prevent duplicate posts
  logReports: boolean; // Whether to log posted reports to file
}
...
export class CostReporterScheduler {
  ...
  private config: CostReporterSchedulerConfig;
  private reportHistory: ReportHistory[] = [];
  private historyPath: string;
  private dailyTimer: ReturnType<typeof setInterval> | undefined;
  private weeklyTimer: ReturnType<typeof setInterval> | undefined;
  private lastDailyReportDate: string = '';
  private lastWeeklyReportDate: string = '';

  constructor(
    log: Logger,
    costReporter: CostReporter,
    dataDir: string,
    config?: Partial<CostReporterSchedulerConfig>
  ) {
    this.log = log;
  ...
  start(discord: DiscordHelper, channelId: string): void {
    ...
    this.dailyTimer = setInterval(() => {
      this.checkAndPostDailyReport().catch(err =>
        this.log.error(`Daily report check failed: ${err}`)
      );
    }, checkIntervalMs);

    this.weeklyTimer = setInterval(() => {
      this.checkAndPostWeeklyReport().catch(err =>
        this.log.error(`Weekly report check failed: ${err}`)
      );
    }, checkIntervalMs);

    this.log.info(
      `CostReporterScheduler started → #trust-debt-public ` +
   ...
   */
  private async checkAndPostDailyReport(): Promise<void> {
    const now = new Date();
   ...
   */
  private async checkAndPostWeeklyReport(): Promise<void> {
    const now = new Date();
   ...
   */
  private async postDailyReport(): Promise<void> {
    try {
   ...
   */
  private async postWeeklyReport(): Promise<void> {
    try {
   ...
   */
  private formatDailyReportForDiscord(report: any): string {
    ...
    if (Object.keys(report.byCategory).length > 0) {
      ...
      const sortedCategories = Object.entries(report.byCategory)
        .sort(([, a]: [string, any], [, b]: [string, any]) => Math.abs(b) - Math.abs(a))
        .slice(0, 5);

      for (const [category, amount] of sortedCategories) {
        const sign = (amount as number) >= 0 ? '+' : '';
        lines.push(`• ${category}: ${sign}$${Math.abs(amount as number).toFixed(4)}`);
      }
      lines.push('');
    }
   ...
   */
  private formatWeeklyReportForDiscord(report: any): string {
    const emoji = report.netBalance >= 0 ? '📈' : '📉';
    const status = report.netBalance >= 0 ? '✅ POSITIVE' : '⚠️ DEFICIT';
   ...
   */
  private recordReportHistory(entry: ReportHistory): void {
  ...
  private loadReportHistory(): void {
    ...
    try {
      ...
      this.reportHistory = lines.map(line => JSON.parse(line) as ReportHistory);
   ...
   */
  getReportHistory(limit: number = 50): ReportHistory[] {
