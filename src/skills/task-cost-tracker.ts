...
export interface TaskCostEntry {
  ...
  outputTokens: number;
  costUSD: number;
  electricityKWh?: number; // For Ollama
  metadata?: {
    agentNumber?: number;
    skillName?: string;
    commandName?: string;
...
export interface TaskCostSummary {
  ...
  backends: Record<string, number>; // backend -> cost
...
export default class TaskCostTracker {
  ...
  trackTaskInference(
    taskId: string,
    taskType: TaskCostEntry['taskType'],
    taskName: string,
    model: string,
    backend: TaskCostEntry['backend'],
    inputTokens: number,
    outputTokens: number,
    metadata?: TaskCostEntry['metadata']
  ): TaskCostEntry {
   ...
   */
  private calculateElectricity(totalTokens: number): number {
  ...
  private appendEntry(entry: TaskCostEntry): void {
    const line = JSON.stringify(entry) + '\n';
    fs.appendFileSync(this.logPath, line, 'utf-8');
  }

  private readEntries(): TaskCostEntry[] {
    if (!fs.existsSync(this.logPath)) {
      return [];
    }
    ...
    for (const line of lines) {
      try {
        entries.push(JSON.parse(line) as TaskCostEntry);
      } catch (err) {
