...
export default class CostReporter {
  ...
  private readTransactions(startTime: number, endTime: number): Transaction[] {
    if (!fs.existsSync(this.ledgerPath)) {
      return [];
    }
  ...
  private generateReport(startTime: number, endTime: number, days: number): Report {
  ...
  private generateSummary(txCount: number, spent: number, income: number, net: number, days: number): string {
  ...
  private async handleTrackInference(
    cmd: unknown,
    ctx: SkillContext
  ): Promise<SkillResult> {
    try {
      const params = cmd as { model: string; inputTokens: number; outputTokens: number };
  ...
  private async handleDailyReport(
    ...
    try {
      const params = cmd as { date?: string };
  ...
  private async handleWeeklyReport(ctx: SkillContext): Promise<SkillResult> {
    try {
  ...
  private async handleAddRevenue(
    cmd: unknown,
    ctx: SkillContext
  ): Promise<SkillResult> {
    try {
      const params = cmd as { source: string; amount: number; description: string };
  ...
  private async handleCheckBudget(
    ...
    try {
      const params = cmd as { sovereignty: number };
