...
import { AgentPool, type TaskPriority } from '../swarm/agent-pool.js';
import {
  getPoolManager,
  executeParallel,
  getPoolStats,
...
} from '../swarm/pool-integration.js';
...
program
  ...
  .action(async (operation: string, options: {
    ...
    const request: ParallelTaskRequest = {
      ...
      targetFiles: files,
      operation: operation as ParallelTaskRequest['operation'],
      priority: options.priority,
      maxConcurrency,
    };
...
async function createMockContext() {
  ...
  return {
    ...
    fs: {
      read: async (path: string) => readFileSync(path, 'utf-8'),
