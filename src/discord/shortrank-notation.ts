...
export interface TesseractCell {
  code: string;      // 'A', 'A1', 'B3', etc.
  emoji: string;     // 🏛️, ⚖️, 📡, etc.
  parent: string;    // 'Strategy', 'Tactics', 'Operations'
  name: string;      // 'Law', 'Goal', 'Signal', etc.
  fullName: string;  // 'Strategy.Law', 'Tactics.Signal', etc.
  trustCategories: string[]; // mapped trust-debt dimensions
  room: string;      // cognitive room this routes to
}

export interface ShortRankIntersection {
  source: TesseractCell;
  target: TesseractCell;
  notation: string;   // '📡 B3 Tactics.Signal : 🔌 C1 Operations.Grid'
  compact: string;    // 'B3:C1'
  isSelfRef: boolean; // source === target
}

/** The 12-cell tesseract grid */
export const TESSERACT_CELLS: Record<string, TesseractCell> = {
  ...
  B1: { code: 'B1', emoji: '🏎️', parent: 'Tactics',    name: 'Speed',      fullName: 'Tactics.Speed',      trustCategories: ['time_management', 'reliability', 'adaptability'], room: 'navigator' },
  ...
  B3: { code: 'B3', emoji: '📡', parent: 'Tactics',    name: 'Signal',     fullName: 'Tactics.Signal',     trustCategories: ['communication', 'transparency', 'innovation'], room: 'voice' },
...
const TRUST_TO_CELL: Record<string, string> = {
  security: 'C1',
  reliability: 'C1',
  data_integrity: 'C1',
...
const CELL_KEYWORDS: Record<string, string[]> = {
 ...
 */
export function detectCell(text: string): { cell: string; confidence: number } {
  ...
  if (entries.length === 0) return { cell: 'B3', confidence: 0.1 }; // Default: Signal
 ...
 */
export function trustCategoriesToCell(categories: string[]): string {
  if (categories.length === 0) return 'B3';
 ...
 */
export function intersection(sourceCode: string, targetCode: string): ShortRankIntersection {
 ...
 */
export function autoIntersection(text: string): ShortRankIntersection {
  const lower = text.toLowerCase();
 ...
 *   🟡 S:58% | #IntentGuard
 */
export function formatTweetWithIntersection(
  text: string,
  sourceCode: string,
  targetCode: string,
  sovereignty: number,
 ...
 */
export function pivotalQuestion(cellCode: string, context: string): {
  question: string;
 ...
 */
export function cellToRoom(cellCode: string): string {
 ...
 */
export function roomToCell(room: string): TesseractCell | undefined {
