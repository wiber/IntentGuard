 ...
 */
export function createPlaygroundBot(logger: Logger, config?: {
  enabled?: boolean;
  port?: number;
  host?: string;
  autoStart?: boolean;
   ...
   */
  const handleCommand = async (message: Message, args: string[]): Promise<void> => {
    ...
    switch (subcommand) {
      ...
      case 'start': {
        ...
        try {
          ...
          await message.reply(
            `❌ Failed to start playground: ${error instanceof Error ? error.message : 'Unknown error'}`
          );
        }
        break;
      }
      ...
      case 'stop': {
        ...
        try {
          ...
          await message.reply(
            `❌ Failed to stop playground: ${error instanceof Error ? error.message : 'Unknown error'}`
          );
        }
        break;
      }
      ...
      case 'restart': {
        ...
        try {
          ...
          await message.reply(
            `❌ Failed to restart playground: ${error instanceof Error ? error.message : 'Unknown error'}`
          );
        }
        break;
      }
 ...
 */
export async function autoStartPlaygroundBot(
  bot: TesseractPlaygroundBot,
  logger: Logger
): Promise<void> {
  try {
 ...
 */
export function registerPlaygroundBotCleanup(
  bot: TesseractPlaygroundBot,
  logger: Logger
): void {
  const cleanup = async () => {
