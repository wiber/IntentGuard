...
function loadConfig(): WrapperConfig {
  ...
  try {
    ...
    if (!token && ocConfig.gateway?.auth?.token) token = ocConfig.gateway.auth.token;
  } catch {}
...
function wireLlmBackends(): void {
  const ocConfigPath = join(OPENCLAW_HOME, 'openclaw.json');
  let config: Record<string, unknown> = {};
  try { config = JSON.parse(readFileSync(ocConfigPath, 'utf-8')); } catch {}

  // Add model configuration for the main agent
  const agentDefaults = (config.agents as Record<string, unknown>)?.defaults as Record<string, unknown> ?? {};
  (agentDefaults as Record<string, unknown>).model = {
    primary: {
      provider: 'ollama',
      model: 'llama3.2:1b',
  ...
  (config.agents as Record<string, unknown>).defaults = agentDefaults;
...
async function connectWebSocket(port: number, token: string): Promise<WebSocket> {
 ...
 */
function handleGatewayMessage(ws: WebSocket, msg: GatewayMessage): void {
  switch (msg.event) {
 ...
 */
async function categorizeFast(text: string): Promise<string[]> {
  try {
    ...
    if (t.includes('test') || t.includes('bug')) categories.push('testing');
    if (t.includes('deploy') || t.includes('build')) categories.push('operations');
    if (t.includes('refactor') || t.includes('design')) categories.push('architecture');
...
async function main(): Promise<void> {
  ...
  if (config.installFimPlugin) installFimPlugin();

  // Step 2: Register skills into OpenClaw workspace
  if (config.registerSkills) registerSkills();

  // Step 3: Wire LLM backends
  if (config.wireLlm) wireLlmBackends();
