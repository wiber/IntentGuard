...
function getMilestoneFile(): string {
 ...
 */
export function recordMeasurement(measurement: SovereigntyMeasurement): void {
  ensureDataDir();
 ...
 */
export function loadHistory(days: number = 90): SovereigntyMeasurement[] {
  ...
  const measurements = lines
    .map(line => {
      try {
        return JSON.parse(line) as SovereigntyMeasurement;
      } catch {
        return null;
      }
    })
    .filter((m): m is SovereigntyMeasurement => m !== null);
 ...
 */
export function getLatestMeasurement(): SovereigntyMeasurement | null {
 ...
 */
export function analyzeStability(config: MonitorConfig = DEFAULT_MONITOR_CONFIG): StabilityAnalysis {
 ...
 */
export function loadMilestones(): StabilityMilestone[] {
 ...
 */
function saveMilestones(milestones: StabilityMilestone[]): void {
  ensureDataDir();
 ...
 */
export function recordMilestone(milestone: StabilityMilestone): void {
 ...
 */
export function hasRecentMilestone(withinDays: number = 30): boolean {
  const milestones = loadMilestones();
...
export class SovereigntyMonitor {
  ...
  private artifactCallback: ((score: number, stableDays: number) => Promise<string>) | null = null;
  private notifyCallback: ((message: string) => Promise<void>) | null = null;

  constructor(log: Logger, config: MonitorConfig = DEFAULT_MONITOR_CONFIG) {
    this.log = log;
    this.config = config;
  }
  ...
  recordFromPipeline(calculation: SovereigntyCalculation): void {
    const measurement: SovereigntyMeasurement = {
   ...
   */
  private buildMilestoneMessage(analysis: StabilityAnalysis, artifactPath?: string): string {
    const lines = [
 ...
 */
export function generateHistoryReport(days: number = 30): string {
 ...
 */
export function exportHistoryCSV(days: number = 90): string {
  const history = loadHistory(days);
