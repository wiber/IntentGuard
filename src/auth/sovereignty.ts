...
export interface SovereigntyCalculation {
  ...
  driftEvents: number;
  rawScore: number; // Before drift reduction
  driftReduction: number; // Percentage reduced by drift
 ...
 */
export function calculateRawSovereignty(trustDebtUnits: number): number {
 ...
 */
export function applyDriftReduction(
  rawSovereignty: number,
  driftEvents: number,
): number {
 ...
 */
export function unitsToGrade(units: number): Grade {
...
export function extractCategoryScores(
  categoryPerformance: TrustDebtStats['category_performance'],
): Partial<Record<TrustDebtCategory, number>> {
  const scores: Partial<Record<TrustDebtCategory, number>> = {};

  // Map pipeline category names to TrustDebtCategory
  const categoryMapping: Record<string, TrustDebtCategory> = {
...
export function calculateSovereignty(
  trustDebtStats: TrustDebtStats,
  driftEvents: number = 0,
): SovereigntyCalculation {
 ...
 */
export function driftEventsUntilZero(currentSovereignty: number): number {
...
export function calculateRecoveryPath(
  currentUnits: number,
  driftEvents: number,
): Array<{ targetGrade: Grade; unitsNeeded: number; sovereigntyGain: number }> {
  const milestones: Array<{ targetGrade: Grade; unitsNeeded: number; sovereigntyGain: number }> = [];
