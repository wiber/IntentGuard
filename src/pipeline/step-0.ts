 ...
 */
function gatherCommits(days: number = 30): RawDocument[] {
  const docs: RawDocument[] = [];

  try {
    const since = new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString();
    const log = execSync(
      `git log --since="${since}" --format="%H|%aI|%s|%b" --name-only`,
 ...
 */
function gatherBlogs(): RawDocument[] {
  const docs: RawDocument[] = [];
  const blogDirs = [
 ...
 */
function gatherDocuments(): RawDocument[] {
  const docs: RawDocument[] = [];
  ...
  if (existsSync(settingsPath)) {
    try {
      ...
      if (settings.documents?.tracked) {
        trackedPaths = Object.values(settings.documents.tracked as Record<string, { path: string }>)
 ...
 */
function gatherVoiceMemos(): RawDocument[] {
  const docs: RawDocument[] = [];
  const memoDir = join(ROOT, 'data', 'attention-corpus');

  if (!existsSync(memoDir)) return docs;
