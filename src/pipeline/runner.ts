...
export async function runPipeline(
  ...
  // Save run summary
  writeFileSync(
    join(runDir, 'pipeline-summary.json'),
 ...
 */
export async function runStep(stepNum: number, runDir: string): Promise<StepResult> {
  ...
  try {
    ...
    if (existsSync(modulePath)) {
      ...
      if (legacyPath) {
        ...
        writeFileSync(
          join(stepDir, `${stepNum}-${name}.json`),
          JSON.stringify({
            step: stepNum,
            name,
            status: 'placeholder',
 ...
 */
function findLegacyScript(stepNum: number): string | null {
  const patterns = [
