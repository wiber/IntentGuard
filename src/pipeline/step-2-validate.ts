...
async function validate() {
  ...
  const hasAllProps = result.categories.every((cat: any) =>
    requiredProps.every(prop => cat.hasOwnProperty(prop)),
  );
  if (hasAllProps) {
  ...
  if (
    result.orthogonality.correlationMatrix &&
  ...
  const totalUnits = result.categories.reduce((sum: number, cat: any) => sum + cat.trustDebtUnits, 0);
  if (totalUnits === 12000) {
  ...
  const hasColors = result.categories.every((cat: any) => /^#[0-9a-f]{6}$/i.test(cat.color));
  if (hasColors) {
  ...
  if (
    result.stats.totalCategories === 20 &&
