...
class IntentGuard {
  ...
  loadConfig() {
    ...
    return {
      ...
      categories: [
        { name: 'Testing', weight: 15, patterns: ['test', 'spec', 'coverage'] },
        { name: 'Documentation', weight: 10, patterns: ['README', 'docs', 'comment'] },
        { name: 'Architecture', weight: 20, patterns: ['structure', 'pattern', 'design'] },
        { name: 'Performance', weight: 15, patterns: ['optimize', 'performance', 'speed'] },
        { name: 'Security', weight: 20, patterns: ['security', 'auth', 'encrypt'] },
        { name: 'User Experience', weight: 20, patterns: ['UX', 'UI', 'interface'] }
      ],
      thresholds: {
        good: 50,
        warning: 100,
    ...
    for (const pattern of this.config.intentDocs) {
      ...
      for (const file of files) {
        ...
        this.config.categories.forEach(category => {
          ...
          intent.categories[category.name] = 
            (intent.categories[category.name] || 0) + count;
        });
    ...
    try {
      ...
      for (const commit of log.all) {
        ...
        this.config.categories.forEach(category => {
          ...
          reality.categories[category.name] = 
            (reality.categories[category.name] || 0) + count;
        });
...
`;
  ...
  async generateHTMLReport(analysis) {
    ...
    let html = template
      ...
      .replace('{{PROJECT}}', path.basename(this.projectDir))
      .replace('{{CATEGORIES}}', JSON.stringify(analysis.categories))
      .replace('{{CONTRIBUTORS}}', JSON.stringify(analysis.topContributors))
      .replace('{{RECOMMENDATIONS}}', JSON.stringify(analysis.recommendations));
